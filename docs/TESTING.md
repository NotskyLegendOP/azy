...
| 2.0f | Settings → *Preset* → `Ultra`, then `Performance`, then `Low power` | Each preset applies instantly to every slider at once and the combo shows what is in effect. Moving any slider afterwards switches the combo to `Custom`; selecting `Custom` changes nothing (it is a state, not a preset). `Ultra` is the only one with accent glow |
| 2.0g | Settings → *Accent* → `Blue`, `Violet`, `Neutral` | The 1px lit edge and the tint change hue immediately. `Neutral` reproduces the pre-1.2 look exactly: white hairline, no colour in the tint. Nothing is ever bright enough to read as neon |
| 2.0h | Settings → *Accent glow* 0 → 100%, then back to 0 | The lit edge brightens; the band does not get wider. At 100% it is still an edge, not a halo. Default is 0% |
| 2.0i | Settings → *Fade Azy's own layers* on, then toggle the skin on/off | At most a short fade of Azy's ring/overlay (80-120 ms). Premiere's own interface does not change either way - it cannot be animated from outside. Off by default |
...
| 2.0j | Settings → Advanced → *Debug mode*, with Premiere open | A click-through outline appears over Premiere: the menu bar and header bands, the left column (Project over Effect Controls), the two monitors, the right dock and the meter strip, the timeline - each labelled with its size, plus a facts block (window, client rectangle, DPI, monitor, workspace). Nothing underneath is blocked: click, drag, scroll and keyboard all still reach Premiere |
| 2.0k | With debug mode on, resize Premiere and move it to another monitor, then change *UI profile* | The outlines follow the window and are rebuilt once it settles; a different profile moves the splits. Log: one `panel map (...)` block per layout change, never one per second |
| 2.0l | Turn debug mode off | The overlay disappears immediately and no `AzySkin.Debug` window remains (check with a window spy or Task Manager → Azy Skin → windows) |
